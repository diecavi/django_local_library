...
class LoanedBookInstancesByUserListViewTest(TestCase):
    def setUp(self):
        ...
        test_language = Language.objects.create(l_code='ENG', lang='English')
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_uses_correct_template(self):
        ...
        # Check we used correct template
        self.assertTemplateUsed(response, 'catalog/renew-book-librarian.html')
...
class CreateNewAuthor(TestCase):
    def setUp(self):
        # Create a user
        test_user1 = User.objects.create_user(username='testuser1', password='1X<ISRUkw+tuK')
        test_user2 = User.objects.create_user(username='testuser2', password='2HJ1vRV0Z&3iD')
        ...
        # Give test_user2 permission to add, change & delete authors
        permission0 = Permission.objects.get(name='Can add author')
        permission1 = Permission.objects.get(name='Can change author')
        permission2 = Permission.objects.get(name='Can delete author')
        test_user2.user_permissions.add(permission0, permission1, permission2)
        test_user2.save()
    ...
    def test_redirects_to_author_detail_on_success(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.post(reverse('catalog:author-create'), {"first_name": "author name", "last_name": "author last" })
        self.assertRedirects(response, reverse('catalog:author-detail', kwargs={"pk": 1}))

    def test_author_changes(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.post(reverse('catalog:author-create'), {"first_name": "author name", "last_name": "author last" })
